...
const MyChats = () => {
  const { user, Chats, setChats, SelectedChat, setSelectedChat, blockedUsers, setBlockedUsers } = ChatState();
  ...
  const handleRightClick = (e, chat) => {
    e.preventDefault();
    console.log("Right clicked!", chat.chatName);
  ...
  try {
    ...
    if (isAlreadyBlocked) {
      ...
      await axios.put("http://localhost:5000/api/user/unblock", {
        userToUnblockId: otherUser._id,
      }, config);
      ...
      await axios.put("http://localhost:5000/api/user/block", {
        userToBlockId: otherUser._id,
      }, config);
